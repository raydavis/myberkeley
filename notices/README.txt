...
3) the servlet extracts the POST properties and hands the to the MessageServiceImpl getting the message node back for writing to the HttpResponse.
...
7) if the MessagePostProcessor finds a changed message node with a messagebox=outbox attribute and a sendstate=none or asendstate=pending attribute, it generates
...
14) MessageTransport aka Handler, e.g. NoticeHandler, then iterates through the routes, e.g. a list of recipient nodes, and creates a new message node under the recipient
