...
for ((a=1; a <= LIMIT ; a++)) 
do
    /usr/bin/curl -u ${FROM_USER}:${FROM_PASS} -F sakai:type=internal -F sakai:sendstate=pending -F sakai:messagebox=outbox  -F sakai:to=internal:${TO_USER} -F sakai:from=${FROM_USER} -F sakai:subject="Test Message.${a}.${now}" -F sakai:body="Test Message Body.${a}.${now}" -F sakai:category=message https://portal-qa.berkeley.edu/user/${FROM_USER}/message.create.html
done